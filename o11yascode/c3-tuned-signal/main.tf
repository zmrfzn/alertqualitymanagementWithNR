...
resource "newrelic_nrql_alert_condition" "GameServingAssets" {
  ...
  # LEARNER ACTION (Challenge 3): raise critical threshold 5 -> 20 to stop false positives, and add expiration/signal-loss handling below.
  critical {
    operator              = "above"
    threshold             = 5
    threshold_duration    = 120
